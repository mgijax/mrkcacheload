...
def deriveAlleleDetailMouseModels(r):
	...
	if humanOrtholog.has_key(marker):
	    ...
	    orthologSymbol = ortholog['orthologSymbol']
            isHumanOrthologAnnotated = humanToDO.has_key(orthologKey)
	else:
	...
	if genotypeOrtholog.has_key(genotype):
	    for s in genotypeOrtholog[genotype]:
	        if s['orthologKey'] != orthologKey:
	            if humanToDO.has_key(s['orthologKey']):
	...
	# check if any human gene is annotated to Term
        isHumanGeneAnnotated = DOToHuman.has_key(termID)
...
def selectHuman(byOrtholog = 0):
	...
	for r in results:
	    key = r['_Marker_key']
	    value = r['termID']
	    if not humanToDO.has_key(key):
		humanToDO[key] = []
	    humanToDO[key].append(value)

	    key = r['termID']
	    value = r['_Marker_key']
	    if not DOToHuman.has_key(key):
		DOToHuman[key] = []
